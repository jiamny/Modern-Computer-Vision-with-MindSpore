# https://obs.dualstack.cn-north-4.myhuaweicloud.com/mindspore-website/notebook/models/application/resnet50_224_new.ckpt
...
momentum = 0.9                              # 动量
workers = 1                                 # 并行线程个数
num_classes = 5                             # 输出通道数大小Flower分类数5

# class_name对应label，按文件夹字符串从小到大的顺序标记label
class_name = {0: "daisy", 1: "dandelion", 2: "roses", 3: "sunflowers", 4: "tulips"}

# 数据集目录路径
data_path_train = "/media/hhj/localssd/DL_data/flower_data2/train/"
data_path_val = "/media/hhj/localssd/DL_data/flower_data2/val/"
...
def _resnet(model_url: str, block: Type[Union[ResidualBlockBase, ResidualBlock]],
    ...
    if pretrained:
        # 加载预训练模型
        if not os.path.exists(pretrianed_ckpt):
...
def resnet50(num_classes: int = 1000, pretrained: bool = False):
    ...
    resnet50_ckpt = "/media/hhj/localssd/DL_data/MindSpore/resnet50/resnet50_224_new.ckpt"
...
def model_train(network, data_loader_train, best_ckpt_dir, best_ckpt_path, num_epochs):
    # 开始循环训练
...
def visualize_model(best_ckpt_path, val_ds, num_classes, class_name):
    net = resnet50()
    # 全连接层输入层的大小
...
#model_train(network, data_loader_train, best_ckpt_dir, best_ckpt_path, num_epochs)
#visualize_model(best_ckpt_path, dataset_val, num_classes, class_name)

# ----------------------------------------
# 固定特征进行训练
#
# 使用固定特征进行训练的时候，需要冻结除最后一层之外的所有网络层。
# 通过设置 requires_grad == False 冻结参数，以便不在反向传播中计算梯度。
# ----------------------------------------
...
def fixFeatures_train(net_work, data_loader_train, best_ckpt_dir, best_ckpt_path, num_epochs):
    # 开始循环训练
    print("Start Training Loop ...")
    best_acc = 0
    for epoch in range(num_epochs):
...
fixFeatures_train(net_work, data_loader_train, best_ckpt_dir, best_ckpt_path, num_epochs)
# 可视化模型预测
visualize_model(best_ckpt_path, dataset_val, num_classes, class_name)
