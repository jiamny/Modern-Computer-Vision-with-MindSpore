...
print('1. Fetching intermediate values by directly calling the intermediate layer')
print(mynet.input_to_hidden_layer(X))


# 2. Fetching intermediate values by returning them in `nn.Module` definition
print('2. Fetching intermediate values by returning them in `nn.Module` definition')
...
def forward(X, Y):
    _Y, _Y_hidden = mynet(X)
    loss = loss_fn(_Y, Y)
    return loss, _Y, _Y_hidden


opt = nn.SGD(mynet.trainable_params(), learning_rate = 0.001)
grad_fn = ms.value_and_grad(forward, None, opt.parameters, has_aux=False)

loss_history = []
for _ in range(50):
    ot = grad_fn(X, Y)
    (loss, _Y, _Y_hidden), grads = grad_fn(X, Y)
    opt(grads)
    loss_history.append(float(loss.item()))
